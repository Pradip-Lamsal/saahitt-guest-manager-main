// Deno Edge Function for password history management
// @ts-expect-error - Deno runtime environment
import { serve } from "https://deno.land/std@0.190.0/http/server.ts";
// @ts-expect-error - ESM import for Deno
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const supabaseUrl =
      (globalThis as DenoEnv).Deno?.env.get("SUPABASE_URL") ?? "";
    const supabaseKey =
      (globalThis as DenoEnv).Deno?.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "";
    ...
    if (action === "check_reuse") {
      ...
      return new Response(JSON.stringify({ isReused: reuseResult }), {
        status: 200,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
